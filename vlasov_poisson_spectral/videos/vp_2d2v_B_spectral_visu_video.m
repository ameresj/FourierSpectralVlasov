...
%         pcolor(x1_pad,x2_pad,fdx.')
        ...
        close all;
        surf(x1_pad,x2_pad,0.*fdx.',fdx.'); 
        
                   
        shading interp;         colormap jet; colorbar off; hold on;
        box off;
           hc=colorbar('Location','south');
            set(hc,'Position', [0.13, 0.75, 0.3,0.03])
            set(hc,'FontSize',10,'TickLength', 0.05)
            view(-35,25);
            axis([0,L(1),0,L(2),0,L(1)/4]); 
            quiver3(L(1)/2,L(2)/2,0,0,0,0.9*L(1)/4,'Linewidth',3,'Color','blue')
