...
splitting='scovel'; % shear std imrotate scovel
...
switch(splitting)
    ...
    case {'scovel','shear'}
        ...
        for sdx=1:length(dtA)
%             if strcmp(splitting,'scovel')
%                 theta=q/m*B3*(dtA(sdx)+dtB(sdx))/subrot;
%             else
%                 theta=q/m*B3*(dtA(sdx))/subrot;
%             end
              theta=q/m*B3*(dtA(sdx)+dtB(sdx))/subrot;
...
%     f=permute(f,[3,4,1,2]);
%     for s=1:9
%     f=imrotate(f,360/9,'bicubic','crop');
%     end
%     f=permute(f,[3,4,1,2]);
